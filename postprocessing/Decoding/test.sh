...
then
    ...
    sed -e :a -e '$!N; s/\n/ /; ta' test.txt |  sed "s/ /\n/1;P;D" | sed "s/^ //g"  > 1test.txt

    python3 decode.py -model / -decoding_graph $graph1 -input_labels input_labels -outfile $graph1.$beam_width.$lmweight\_hyps.txt -test 1test.txt -beam_width $beam_width -lmweight $lmweight -search $search
