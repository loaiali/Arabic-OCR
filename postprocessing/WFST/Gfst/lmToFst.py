...
def main():
    ...
    parser.add_argument(
        '-out_dir', help='we will write output to here, adding .tfsa and .syms extensions')
    parser.add_argument('--prune_5k', '-prune_5k',
                        help='artificially limit to the top 5000 unigrams in the model', action='store_true')
    args = parser.parse_args()

    lm = arpalm(args.lmfile)
    lm.make_fst(args.prune_5k)
    lm.write_fst_body(args.out_dir + '.txt')
    lm.write_fst_symbols('input.syms')
...
class arpalm:
    ...
    def make_fst(self, prune_5k=False):
        ...
        if prune_5k:
            if len([x for x in self.ngrams if len(x) == 1]) <= 5000:
                ...
                unigram_thresh = sorted(
                    [self.ngrams[x][0] for x in self.ngrams if len(x) == 1], reverse=True)
                unigram_thresh = unigram_thresh[5000]
                unigrams = [x[0] for x in self.ngrams if len(
                    x) == 1 and self.ngrams[x][0] > unigram_thresh]
        ...
        for ngram in self.ngrams:
            ...
            if score[0] > -99:  # this number means 0 probabilty set by SLRM library
                arcs.append((src, dst, word, label,
                             -np.log10(np.exp(1)) * score[0]))
        ...
        for ngram in filter(lambda x: len(x) > 1, state_map):
            ...
            if pretend_ngram not in self.ngrams:
                # make sure we don't need recursion
                assert(pretend_ngram[1:] in self.ngrams)
    ...
    def write_fst_body(self, fst_body_filename):
        with open(fst_body_filename, 'wt') as f:
            f.write('\n'.join(['{:d} {:d} {} {} {:f}'.format(
                int(x[0]), int(x[1]), x[2], x[3], float(x[4])) for x in self._fst]))
            f.write('\n')
            f.write('\n'.join(['{:d}'.format(int(x))
                               for x in self._end_states]))
